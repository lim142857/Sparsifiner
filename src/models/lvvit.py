...
class Attention(nn.Module):
    ...
    def forward(self, x, policy, padding_mask=None, return_attn=False):
        B, N, C = x.shape
...
class Block(nn.Module):
    ...
    def forward(self, x, policy=None, padding_mask=None, return_attn=True):
        if return_attn:
            new_x, attn = self.attn(self.norm1(x), policy, padding_mask, return_attn)
...
class MHABlock(nn.Module):
    ...
    def forward(self, x, padding_mask=None, return_attn=True):
        if return_attn:
            new_x, attn = self.attn(self.norm1(x * self.skip_lam), padding_mask, return_attn=return_attn)
            x = x + self.drop_path(new_x) / self.skip_lam
            return x, attn
        else:
...
class LVViTTeacher(nn.Module):
    ...
    def forward(self, x):
        return_attn = self.cfg.VIT.RETURN_ATTN  # Modified
        ...
        feature = self.norm(x)
        cls = self.head(feature[:, 0])
        x_aux = self.aux_head(feature[:, 1:])
        tokens = x_aux
