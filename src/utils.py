...
def linear_scheduler(max_value, epochs, niter_per_ep):
    total_steps = epochs * niter_per_ep
    schedule = np.array([max_value * (1 - i / total_steps) for i in range(total_steps)])
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, best_acc, best_acc_ema, model_ema=None):
    ...
    for checkpoint_path in checkpoint_paths:
        ...
        # Mlflow or Wandb or Others
        # if is_main_process():
        #     if not isinstance(epoch, int):  # for best and ema
        #         # Save checkpoint here
        #     elif epoch % args.mlflow_ckpt_save_freq == 0:
        #         # Save checkpoint here
...
def batch_index_fill_x1(x, x1, idx1):
    B, N, C = x.size()
    B, N1, C = x1.size()
